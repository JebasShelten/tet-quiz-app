...
export default function Quiz({ bankId, onBack }) {
  ...
  // NEW: State to track bookmarked question IDs
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (user) {
        ...
        if (bData) {
          setBookmarkedIds(new Set(bData.map(b => b.question_id)));
        }
        ...
        if (savedProgress) {
          ...
          if (savedProgress.answers) {
            ...
            if (answeredKeys.length > 0) {
              ...
              if (lastAnswered < qData.length - 1) setCurrentIndex(lastAnswered + 1);
              else setCurrentIndex(lastAnswered);
            }
          }
  ...
  const handleOptionClick = async (letter) => {
    ...
    if (user) {
      if (resultId) {
        await supabase.from('quiz_results').update({ answers: newAnswers, status: 'in_progress' }).eq('id', resultId);
      } else {
        const { data } = await supabase.from('quiz_results').insert([{ user_id: user.id, bank_id: bankId, answers: newAnswers, status: 'in_progress', score: 0, total_questions: questions.length }]).select().single();
  ...
  const handleSubmitQuiz = async () => {
    ...
    questions.forEach((q, index) => {
      if (userAnswers[index] === (q.final_key || q.correct_option)) calculatedScore += 1;
    });
    setFinalScore(calculatedScore);

    const { data: { user } } = await supabase.auth.getUser();
    if (user && resultId) {
      await supabase.from('quiz_results').update({ answers: userAnswers, status: 'completed', score: calculatedScore }).eq('id', resultId);
    }
  ...
  const handleRetry = async () => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (user && resultId) {
      await supabase.from('quiz_results').update({ answers: {}, status: 'in_progress', score: 0 }).eq('id', resultId);
    }
    setIsFinished(false);
    setIsSubmitting(false);
  };
    ...
    return (
      <div className="max-w-3xl mx-auto mt-10">
        <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-xl text-center flex flex-col items-center">
          <div className="w-24 h-24 bg-violet-100 rounded-full flex items-center justify-center mb-6 shadow-inner text-3xl">🏆</div>
          ...
          <div className="flex gap-4">
            <button onClick={onBack} className="px-8 py-4 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition cursor-pointer">Back to Dashboard</button>
            <button onClick={handleRetry} disabled={isSubmitting} className="px-8 py-4 bg-violet-600 text-white font-bold rounded-xl hover:bg-violet-700 transition shadow-md cursor-pointer">Retry Quiz</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto flex flex-col xl:flex-row gap-6 pb-10">
      <div className={`flex-1 flex flex-col gap-6 transition-transform ${shake ? 'translate-x-2' : ''}`}>
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-violet-600 transition font-medium cursor-pointer">
            <ArrowLeft size={20} /> Back to Dashboard
          </button>
          <div className="flex items-center gap-2 bg-violet-50 text-violet-700 px-4 py-2 rounded-lg font-bold"><Clock size={20} /> 150:00</div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 md:p-10 border border-gray-100 shadow-sm relative min-h-[500px] flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                ...
                className={`flex items-center gap-1 text-sm font-bold transition transform active:scale-95 cursor-pointer
                  ${isBookmarked ? 'text-orange-500' : 'text-gray-400 hover:text-orange-500'}`}
              >
                <Bookmark size={18} fill={isBookmarked ? "currentColor" : "none"} /> {isBookmarked ? 'Bookmarked' : 'Bookmark'}
              </button>
            </div>

            <div className="w-full bg-gray-100 h-2 rounded-full mb-8 overflow-hidden">
              <div className="bg-violet-600 h-full transition-all duration-500 ease-out" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }}></div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {['A', 'B', 'C', 'D'].map((letter) => {
                ...
                return (
                  <button key={letter} onClick={() => handleOptionClick(letter)} className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 transition-all duration-300 text-left group ${btnClass}`}>
                    <span className={`w-10 h-10 flex-shrink-0 font-bold rounded-full flex items-center justify-center transition-colors ${iconClass}`}>{letter}</span>
          ...
          <div className="flex justify-between items-center mt-10 pt-6 border-t border-gray-100">
            <button onClick={() => setCurrentIndex(prev => Math.max(0, prev - 1))} disabled={currentIndex === 0} className="px-6 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100 disabled:opacity-30 transition transform active:scale-95 cursor-pointer">Previous</button>
            {currentIndex === questions.length - 1 ? (
              <button onClick={handleSubmitQuiz} disabled={isSubmitting} className="px-8 py-3 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-600 transition shadow-md flex items-center gap-2 transform active:scale-95 cursor-pointer">{isSubmitting ? 'Saving...' : 'Submit Quiz'}</button>
            ) : (
              <button onClick={() => setCurrentIndex(prev => Math.min(questions.length - 1, prev + 1))} className="px-6 py-3 bg-violet-600 text-white rounded-xl font-bold hover:bg-violet-700 transition shadow-md flex items-center gap-2 transform active:scale-95 cursor-pointer">Next <ArrowRight size={20} /></button>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col h-[600px]">
          ...
          <div className="grid grid-cols-5 gap-2 overflow-y-auto pr-2 custom-scrollbar flex-1 pb-4">
            {questions.map((q, i) => {
              ...
              if (isAnswered) navClass = isCorrect ? "bg-emerald-500 text-white shadow-sm" : "bg-red-500 text-white shadow-sm";
              if (i === currentIndex) navClass = "bg-white text-violet-600 border-2 border-violet-500 shadow-md transform scale-110";
              return (
                <button key={q.id} onClick={() => setCurrentIndex(i)} className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-200 border-2 cursor-pointer ${navClass}`}>{i + 1}</button>
              );
            })}
          </div>
        </div>
